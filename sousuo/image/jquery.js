...
(function(a) {
	function b(b, c, d) {
		var e = this,
			...
			g = "beforeSwitch",
			h = "onSwitch";
		a.isFunction(c[g]) && f.bind(g, c[g]), a.isFunction(c[h]) && f.bind(h, c[h]), a.extend(e, {
			...
			_init: function() {
				...
				if ( !! c.triggers) {
					var f, g, h;
					if (c.triggers.jquery) {
						e.triggers = c.triggers.slice(0, e.length)
					} else {
						var i = a.type(c.triggers) === "string",
							j = [];
						for (g = 1; g <= e.length; g++) {
							j.push("<li>" + (i ? c.triggers : g) + "</li>")
						}
						e.triggers = a("<ul/>", {
					...
					if (c.slide) {
						var n = e.triggers.eq(e.index),
							w = (n.width() + parseInt(n.css("padding-left")) + parseInt(n.css("padding-right"))) / 2 + n.position().left -13;
						n.siblings("span").animate({
							left: w
						}, 100)
					}
					for (g = 0; g < e.length; g++) {
						f = e.triggers.eq(g), f.click({
							index: g
						}, function(a) {
							h = a.data.index;
							if (!e._triggerIsValid(h)) {
								return
							}
							...
							e._delayTimer = setTimeout(function() {
								if (c.slide) {
									var w = (n.width() + parseInt(n.css("padding-left")) + parseInt(n.css("padding-right"))) / 2 + n.position().left - 10;
									n.siblings("span").animate({
										left: w
									}, 100)
								}
			...
			_switchTrigger: function(a, b) {
				e.triggers.eq(a).removeClass(c.currentTriggerCls).end().eq(b).addClass(c.currentTriggerCls)      //从这个元素移动到下一个元素     e.triggers.eq(b)等于当前元素“a”
				if (c.slide) {
					var n = e.triggers.eq(b),
						w = (n.width() + parseInt(n.css("padding-left")) + parseInt(n.css("padding-right"))) / 2 + n.position().left -10;
					n.siblings("span").animate({
						left: w
					}, 100)
				};//点击箭头三角滑动_jxt
	...
	function b() {
		...
		if (a.style[c] !== undefined) {
			...
			for (e = 0; e < 2; e++) {
				if (a.style[c = b[e] + "Transition"] !== undefined) {
					d = c;
					break
				}
			}
	...
	a.switchable.Anim = function(c, d, e, f, g, h) {
		var i = this,
			j = {},
			k, l;
		a.switchable.Transition === undefined && (a.switchable.Transition = b()), k = a.switchable.Transition, a.extend(i, {
			isAnimated: !1,
			run: function() {
				if (i.isAnimated) {
					return
				...
				e = e * 1000;
				if (k) {
					j[k + "Property"] = h || "all", j[k + "Duration"] = e + "ms", j[k + "TimingFunction"] = f, c.css(a.extend(d, j)), l = setTimeout(function() {
					...
					if (m || g) {
						f = a.switchable.Easing(g ? g[1] : m.match(b)[1])
					}
	...
	a.switchable.TimingFn = {
		...
		if (g !== 4) {
			window.console && console.warn(b(e.join(", ")) + " missing argument.")
		} else {
		...
		init: function(b) {
			...
			function i() {
				f = setInterval(function() {
					h()
				}, (c.interval + c.duration) * 1000)
			}
			var c = b.config,
				d = !1,
				e, f, g;
			if (!c.autoplay || b.length <= 1) {
				return
			}
	...
	a.extend(a.switchable.Config, {
		...
		init: function(b) {
			var c = b.config,
				...
				e = ["prev", "next"],
				f, g, h, i = 0;
			var custom = c; //yangkai
			if (!c.prev && !c.next) {
				return
			}
			for (; i < 2; i++) {
				f = e[i], g = c[f], g && (h = b[f + "Btn"] = g.jquery ? g : a(g), h.click({
					direction: d[i]
				}, function(a) {
					a.preventDefault();
					if (!b.anim) {
						...
						if (custom.roll) {
							...
							var oPic = b.container[0];
							var oList = b.triggers.parent().parent()[0];
							var oPicLi = b.panels;
							var oListLi = b.triggers;
							var len1 = oPicLi.length;
							var len2 = oListLi.length;
							var oPicUl = oPic;
							var oListUl = oList.getElementsByTagName("li")[0];
							var w1 = oPicLi[0].offsetWidth;
							var w2 = oListLi[0].offsetWidth;
							var num = b.config.numm;
							var num2 = Math.ceil(num / 2+4);
							function Change() {
								var c = a.data.direction
								animate(oPicUl, {left: - b.index * w1});
								if (b.index <= num2) {
									animate(oListUl, {left: 0});
								} else if (b.index + num2 <= len2) {
									animate(oListUl, {left: -(b.index - num2) * w2});
								} else {
	...
	for (var f = 0; f < 4; f++) {
		a.switchable.Effects[b[f]] = function(b, c, d) {
			var e = this,
				f = e.config,
				g = e.length - 1,
				h = d === "backward",
		...
		init: function(f) {
			...
			if (g.end2end) {
				var s = i.length,
					...
					x;
				g.loop = !0, g.visible && g.visible < s && g.visible > u && i.slice(0, g.visible).clone(!0).addClass(g.clonedCls).appendTo(j).click(function(b) {
					b.preventDefault(), i.eq(a(this).index() - s).click()
				}), a.extend(f, {
	...
	for (var d = 0; d < 2; d++) {
		a.switchable.Effects[b[d]] = function(b, c) {
			var d = this,
				e = d.config,
				f = b !== c;
	...
	a.switchable.Plugins.push({
		...
		init: function(d) {
			...
			window.console && console.info("Remember to set the border-width for the accordion's panels, even without border."), a.extend(d, {
				_triggerIsValid: function(a) {
					return !0
				},
				_switchTrigger: function(a, b) {
			...
			for (k = 0; k < d.length; k++) {
				...
				d.expandProps.push(a.extend({}, h)), i.css(a.extend({
					overflow: "hidden"
...
/*gotop*/
(function($){$.fn.gotop=function(options){var bodywidth=$(".auto").width();var settings=$.extend({"content":bodywidth,"bottom":0,"margin":"none","position":"right","scrollTop":1000,"duration":100,"anchora":false},options);var win=$(window);var top=this;function hasPlace(){var place=true;if(settings.margin=="none"){if(win.width()<(settings.content+(top.width()*2)+4)){place=false}}else{if(win.width()<(settings.content+((top.width()+settings.right)*2)+4)){place=false}}return place}function placeTop(){var pos=(((win.width()-settings.content)/2)-top.width())-5;if(settings.position=="left"||settings.position=="l"){top.css({"left":pos+"px"})}else{top.css({"right":pos+"px"})}}function showTop(){if(win.scrollTop()>settings.scrollTop){top.fadeIn();if($.browser.msie&&($.browser.version=="6.0")){var thisTop=win.scrollTop()+win.height()-top.height()-settings.bottom;top.css("top",thisTop+"px")}}else{top.fadeOut()}}if(!hasPlace()){this.hide()}if(settings.margin=="none"){placeTop()}else{if(settings.position=="left"||settings.position=="l"){top.css({"left":settings.margin+"px"})}else{top.css({"right":settings.margin+"px"})}}showTop();win.resize(function(){if(hasPlace()){top.fadeIn()}else{top.fadeOut()}if(settings.margin=="none"){placeTop()}});win.scroll(function(){showTop()});if(settings.anchora){if($.browser.msie&&($.browser.version=="6.0")){top.css({"position":"absolute"})}else{top.css({"position":"fixed","bottom":settings.bottom+"px"})}}else{if($.browser.msie&&($.browser.version=="6.0")){top.css({"position":"absolute","cursor":"pointer"})}else{top.css({"position":"fixed","cursor":"pointer","bottom":settings.bottom+"px"})}return $(".x_gt_a2").click(function(){$("body,html").animate({scrollTop:0},settings.duration)})}}})(jQuery);
